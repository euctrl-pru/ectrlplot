...
create_footer <- function(source_name, logo_image_path) {
    # Make the footer
    footer <- grid::grobTree(grid::linesGrob(x = grid::unit(c(0, 1), "npc"), y = grid::unit(1.1, "npc")), grid::textGrob(source_name, x = 0.004,
        hjust = 0, gp = grid::gpar(fontsize = 16)), grid::rasterGrob(png::readPNG(logo_image_path), x = 0.944))
    return(footer)

}

#' Arrange alignment and save PRU ggplot chart
#'
#' Running this function will save your plot with the correct guidelines for publication for a PRU graphic.
#' It will left align your title, subtitle and source, add the PRU blocks at the bottom right and save it to your specified location.
...
#'  which needs to be a PNG file - defaults to PRU blocks image that sits within the data folder of your package
...
#'
#' @export
add_logo <- function(plot_name, source_name, save_filepath = file.path(Sys.getenv("TMPDIR"), "tmp-nc.png"), width_pixels = 640, height_pixels = 450,
    logo_image_path = file.path(system.file("data", package = "ectrlplot"), "ectl_logo.png")) {
